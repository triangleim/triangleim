...
*/
long long addHyperedge(Graph & g, HyperGraph & hg, int t, long long num, bool lt) // num=curSamples
{
        ...
        #pragma omp parallel
	{
		...
		if (lt == 0){
			...
			// cout<<endl;
			while (iter < num){ // 进行num为10000为单位（即舍去末位变0）的sample
								// cout<<"\r";
								// cout<<iter<<"/"<<num;
                                for (int i = 0; i < c; ++i){
                                        vector<int> he;
                                        hg.pollingIC1(g,visit,visit_mark); // 1次RR集合生成
	                        }
                        	#pragma omp atomic
                        	iter += c;
                	}
