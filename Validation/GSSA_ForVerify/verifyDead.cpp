...
#pragma omp parallel
    {
        ...
        if (m == 0)
        {
            ...
            while (counter <= maxSamples)
            {
                ...
                maxSeed[id] = hg.pollingLT3(g, link, k, visit, visit_mark,node1,node2,node3);
#pragma omp critical
                {
                    counter++;
                    if (maxSeed[id])
                    {
                        degree++;
                        dead += g.deads[node1];
                        lifetime += g.lifetimes[node1];
                        view += g.views[node1];
                        dead += g.deads[node2];
                        lifetime += g.lifetimes[node2];
                        view += g.views[node2];
                        dead += g.deads[node3];
                        lifetime += g.lifetimes[node3];
                        view += g.views[node3];
                    }
                }
            }
        ...
        else
        {
            ...
            while (counter <= maxSamples)
            {
                ...
                maxSeed[id] = hg.pollingIC3(g, link, k, visit, visit_mark,node1,node2,node3);
#pragma omp critical
                {
                    counter++;
                    if (maxSeed[id])
    ...
    cout<< dead/(double)degree/3<<endl;
    cout<< (unsigned long long)(view/(double)degree/3)<<endl;
    cout<< lifetime/(double)degree/3<<endl;
...
int main(int argc, char **argv)
{
    ...
    // 	t = atoi(tmp);
    // }
    char* indexFile = op.getPara("-nodescc");
    long long nt = g.getTriNumber();
