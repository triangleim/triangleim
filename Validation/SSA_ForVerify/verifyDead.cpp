...
#pragma omp parallel
    {
        ...
        if (m == 0)
        {
            ...
            while (counter <= maxSamples)
            {
                ...
                maxSeed[id] = hg.pollingLT3(g, link, k, visit, visit_mark);
#pragma omp critical
                {
                    counter++;
                    if (maxSeed[id])
                    {
                        degree++;
                        dead += g.deads[maxSeed[id]];
                        lifetime += g.lifetimes[maxSeed[id]];
                        view += g.views[maxSeed[id]];
                    }
                }
            }
        }
        else
        {
            ...
            while (counter <= maxSamples)
            {
                ...
                maxSeed[id] = hg.pollingIC3(g, link, k, visit, visit_mark);
#pragma omp critical
                {
                    counter++;
                    if (maxSeed[id])
    ...
    cout<< dead/(double)degree<<endl;
    cout<< view/(double)degree<<endl;
    cout<< lifetime/(double)degree<<endl;
