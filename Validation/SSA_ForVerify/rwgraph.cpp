...
int HyperGraph::pollingIC3(Graph &g, vector<unsigned int> & link, unsigned int k, vector<bool> &visit, vector<int> &visit_mark)
{
        int i;
        unsigned int cur = sfmt_genrand_uint32(&sfmtSeed)%(g.getSize())+1;
        int node=cur;
...
int HyperGraph::pollingLT3(Graph &g, vector<unsigned int> & link, unsigned int k, vector<bool> &visit, vector<int> &visit_mark)
{
        unsigned int i;
        bool t = false;
        unsigned int gSize = g.getSize();
