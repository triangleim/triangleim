...
public:
	...
	static void load_graph(const std::string graphName, const bool isReverse,GraphInfo& graphinfo,const std::string probDist = "load",
	                        const float probEdge = float(0.1))
	{
		TIO::load_graph_struct(graphName, graphinfo, isReverse);
		...
		if (tolower(probDist[0]) == 'w')
		{
			...
			{
				// Reverse graph
				for (auto& nbrs : graphinfo.graph)
				{
					for (auto& nbr : nbrs)
					{
						// float newv = 1.0 / nbrs.size();
						nbr.second = float(1.0 / nbrs.size());
						// if(newv - nbr.second>1e-6 || newv - nbr.second<-1e-6 ){
						// 	std::cout<<"Wrong\t"<<newv<<"\t"<<nbr.second<<std::endl;
						// 	nbr.second = float(1.0 / nbrs.size());
			...
			{
				// Forward graph
				std::vector<uint32_t> vecInDeg(graphinfo.numNodes);
				for (auto& nbrs : graphinfo.graph)
				{
					for (auto& nbr : nbrs)
					{
