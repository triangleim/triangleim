...
public:
	...
	unsigned long long _triangle=1;
	...
	std::string _dir = "../datasets"; // Directory
	...
	Argument(int argc, char* argv[])
	{
		...
		for (int ind = 1; ind < argc; ind++)
		{
			...
			else if (!param.compare("-triangle")) _triangle = stoull(value);
		}
		...
		else if (_mode == "3" || _mode == "avg")
		{
			postfix = "_avgBound";
		}
		_outFileName = TIO::get_out_file_name(_graphname, _algName + postfix, _seedsize, _probDist, _probEdge);
		if (_model == LT) _outFileName = _outFileName+"_LT";
		else if(_model == IC)_outFileName = _outFileName+"_IC";
