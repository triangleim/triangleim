        ...
        for (unsigned int k = 0; k < 3; ++k)
        {
            ...
            for (unsigned int j = 0; j < num2; ++j)
            {
                ...
                if (node_edge[edge_node[i][k][j]].empty() || node_edge[edge_node[i][k][j]][node_edge[edge_node[i][k][j]].size() - 1] != i)
                    node_edge[edge_node[i][k][j]].push_back(i); // This node appears in which rr sequences.
...
void HyperGraph::pollingLT1(Graph &g, vector<bool> &visit, vector<int> &visit_mark)
{
    ...
    for (i = 0; i < gSize; ++i)
    {
        if (visit12[cur] == true)
        {
            intersect_flag2 = find(visit_mark.begin(), visit_mark.begin() + num_marked2, cur) - visit_mark.begin();
            break;
        }
        if (visit[cur] == true)
        {
...
void HyperGraph::pollingIC1(Graph &g, unordered_map<int, bool> &visit, vector<int> &visit_mark)
{
    ...
    if (true)
    {
        int subsize = rrset1.size() + rrset2.size() + rrset3.size();
        {
    ...
    result.push_back(rrset3);
    visit.clear();
    edge_node.push_back(vector<vector<int>>(result));
    // return;
    // cout<<"\rHere2"<<flush;
}
