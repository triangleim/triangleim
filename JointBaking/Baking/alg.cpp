...
double Alg::max_cover_lazyUpper(const int targetSize, const int mode)
{
	...
	__boundLastU = DBL_MAX, __boundMinU = DBL_MAX;
	FRset coverage(__numV, 0);
	size_t maxDeg = 0;
	for (auto i = __numV; i--;)
	{
		const auto deg = __hyperG._FRsetsU[i].size();
		coverage[i] = deg;
		if (deg > maxDeg)
			maxDeg = deg;
	}
...
// std::cout<<"here7"<<std::endl;
	...
	for (auto deg = maxDeg; deg > 0; deg--) // Enusre deg > 0
	{
		...
		for (auto idx = vecNode.size(); idx--;)
		{
			...
			if (mode == 2 || (mode == 1 && __vecSeedU.size() == targetSize))
			{
				// Find upper bound
				...
				__boundLastU = double(accumulate(vecBound.begin(), vecBound.end(), size_t(0)) + sumInf) * __numT / __numRRsets;
				if (__boundMinU > __boundLastU)
					__boundMinU = __boundLastU;
			}
			// // std::cout<<"here8"<<std::endl;
			if (__vecSeedU.size() >= targetSize)
			{
				// Top-k influential nodes constructed
				const auto finalInf = 1.0 * sumInf * __numT / __numRRsets;
				std::cout << "  >>>[greedy-lazy] influence: " << finalInf << ", min-bound: " << __boundMinU << ", last-bound: " << __boundLastU << '\n';
				return finalInf;
			}
			// std::cout<<"here9"<<std::endl;
			sumInf += currDeg;
			__vecSeedU.push_back(argmaxIdx);
			coverage[argmaxIdx] = 0;
			// std::cout<<"here10"<<std::endl;
			for (auto edgeIdx : __hyperG._FRsetsU[argmaxIdx])
			{
				if (edgeMark[edgeIdx])
					continue;
				edgeMark[edgeIdx] = true;
				// std::cout<<edgeIdx<<" "<<__hyperG._RRsetsU.size()<<std::endl;

				for (auto nodeIdx : __hyperG._RRsetsU[edgeIdx])
				{
...
double Alg::max_cover_topkUpper(const int targetSize)
{
	...
	for (auto k = targetSize; k--;)
	{
		...
		__vecSeedU.push_back(seed);
		coverage[seed] = 0;
		for (auto edgeIdx : __hyperG._FRsetsU[seed])
		{
			if (edgeMark[edgeIdx])
				continue;
			edgeMark[edgeIdx] = true;
		...
		__boundLastU = 1.0 * (sumInf + sumTopk);
		if (__boundMinU > __boundLastU)
			__boundMinU = __boundLastU;
	}
	__boundMinU *= 1.0 * __numT / __numRRsets;
	__boundLastU *= 1.0 * __numT / __numRRsets;
	const auto finalInf = 1.0 * sumInf * __numT / __numRRsets;
	std::cout << "  >>>[greedy-topk] influence: " << finalInf << ", min-bound: " << __boundMinU << ", last-bound: " << __boundLastU << '\n';
	return finalInf;
}
...
double Alg::max_cover_topk(const int targetSize)
{
	...
	__boundMin *= 1.0 * __numT / __numRRsets;
	__boundLast *= 1.0 * __numT / __numRRsets;
	const auto finalInf = 1.0 * sumInf * __numT / __numRRsets;
	std::cout << "  >>>[greedy-topk] influence: " << finalInf << ", min-bound: " << __boundMin << ", last-bound: " << __boundLast << '\n';
	return finalInf;
}
...
double Alg::max_coverUpper(const int targetSize, const int mode)
{
	if (targetSize >= 1000)
		return max_cover_topkUpper(targetSize);
	return max_cover_lazyUpper(targetSize, mode);
}
...
// std::cout<<"here2"<<std::endl;
		...
		if (!__hyperG.stopUpper)
		{
			const auto infSelfU = max_coverUpper(targetSize, mode);
			const auto infVldtU = __hyperGVldt.self_inf_calU(__vecSeedU);
			const auto degVldtU = infVldtU * __numRRsets / __numT;
			auto upperBoundU = infSelfU / approx;
			if (mode == 1)
				upperBoundU = __boundLastU;
			else if (mode == 2)
				upperBoundU = __boundMinU;
			const auto upperDegOPTU = upperBoundU * __numRRsets / __numT;
			const auto lowerSelectU = pow2(sqrt(degVldtU + a1 * 2.0 / 9.0) - sqrt(a1 / 2.0)) - a1 / 18.0;
			const auto upperOPTU = pow2(sqrt(upperDegOPTU + a2 / 2.0) + sqrt(a2 / 2.0));
			const auto approxOPIMCU = lowerSelectU / upperOPTU;
			if ((approxOPIMCU >= approx - epsilon || (time1 + time2) * 2 > TIMELIMIT)) // alpha>=1-1/e
			{
				__hyperG.stopUpper = true;
				__tRes.set_seed_vec2(__vecSeedU);
			}
		}
		// guaranteeOpt = opt_error(degVldt, degSelfUpper, delta);
		...
		if (approxOPIMC >= approx - epsilon || (time1 + time2) * 2 > TIMELIMIT) // alpha>=1-1/e
		{
