...
public:
	...
	static void write_order_seeds2(const std::string &outFileName, const TResult &resultObj, const std::string &outFolder, const std::string &cascadeModel = "IC")
	{
		auto vecSeed = resultObj.get_seed_vec2();
		mkdir_absence(outFolder.c_str());
		const auto outpath = outFolder + "/seed";
		mkdir_absence(outpath.c_str());
		std::ofstream outFile(outpath + "/seed_upper_" + outFileName);
		for (auto i = 0; i < vecSeed.size(); i++)
		{
			outFile << vecSeed[i]+1 << '\n';
		}
