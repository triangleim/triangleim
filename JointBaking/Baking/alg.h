...
private:
	...
	/// Upper bound in the last round for __mode=1.
	double __boundLastU = DBL_MAX;
	/// The minimum upper bound among all rounds for __model=2.
	double __boundMinU = DBL_MAX;
	...
	/// Maximum coverage by lazy updating.
	double max_cover_lazyUpper(const int targetSize, const int mode = 2);
	/// Maximum coverage by maintaining the top-k marginal coverage.
	double max_cover_topkUpper(const int targetSize);
	/// Maximum coverage.
	double max_coverUpper(const int targetSize, const int mode = 2);
public:
	Alg(GraphInfo& graphinfo, TResult& tRes) : __hyperG(graphinfo), __hyperGVldt(graphinfo), __tRes(tRes)
	{
		__numV = __hyperG.get_nodes();
		__numE = __hyperG.get_edges();
		__numT = __hyperG.get_triangles();
	}
	~Alg()
	{
	}
